#to run: python3 asymmetric_coord_game_select_slot.py
...
class Agent():
	...
	def __init__(self, agent_id, no_strats, init_prob_play_0, init_prob_play_left, init_prob_imi_own_play_group):
...
#		self.group=poss_group_members[agent_id]
		...
		self.prob_play_left=init_prob_play_left[agent_id]
	...
	def Update_Strat_Prob(self, all_agents, mut):
		...
		q1=self.prob_play_left
		
		q2=all_agents[self.agent_to_imitate].prob_play_left
...
init_prob_play_left_tmp=0.8

init_prob_imi_own_play_group=0.9

init_prob_play_0=np.random.random(no_agents)

init_prob_play_left=np.random.random(no_agents)#np.random.choice([0, 1], size=no_agents, p=[init_prob_play_left_tmp, 1-init_prob_play_left_tmp])#

init_prob_imi_own_play_group=np.ones(no_agents)*init_prob_imi_own_play_group

all_agents=[]

for sel_agent in np.arange(no_agents):

	all_agents.append(Agent(sel_agent, no_strats, init_prob_play_0, init_prob_play_left, init_prob_imi_own_play_group))
...
for time_step in np.arange(no_time_steps):
	...
	for sel_agent in agent_order:
		...
		q=all_agents[sel_agent].prob_play_left
...
#		print("sel_agent_to_imitate = ",sel_agent_to_imitate)
	...
	for sel_agent in np.arange(no_agents):
		...
		all_qs[sel_agent]=all_agents[sel_agent].prob_play_left
...
#	print(left_group_inds)
	...
	mean_payoff_0=np.mean(all_payoffs[left_group_inds])
	mean_payoff_1=np.mean(all_payoffs[right_group_inds])
	
	mean_p=np.mean(all_ps)
	mean_p_0=np.mean(all_ps[left_group_inds])
	mean_p_1=np.mean(all_ps[right_group_inds])
	
	mean_q=np.mean(all_qs)
	mean_q_0=np.mean(all_qs[left_group_inds])
	mean_q_1=np.mean(all_qs[right_group_inds])
	
	mean_w=np.mean(all_ws)
	mean_w_0=np.mean(all_ws[left_group_inds])
	mean_w_1=np.mean(all_ws[right_group_inds])
