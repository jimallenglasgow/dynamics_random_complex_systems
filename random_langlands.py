#to run: python3 random_langlands.py
...
def Calc_b_dot(b):
	...
	for i in np.arange(1, no_eqs):
		...
		coeff=sp.special.factorial(2*(i-1)+1)*alpha_sign[i]
		
		print(coeff)
	
#		b_dot_tmp=b_dot_tmp+(1/coeff)*(10*alpha[i]*b[0])**(2*(i-1)+1)

		b_dot_tmp=b_dot_tmp+(10*alpha[i]*b[0])**(2*(i-1)+1)
...
for b1 in np.arange(0,1.01,0.02):
