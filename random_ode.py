...
def Calc_b_dot(b):
	...
	no_roots=len(alpha[0, :])
	
#	print("no_roots = ",no_roots)

	for sel_ind in np.arange(no_eqs):
	
		b_dot_tmp=1
	
		for sel_ind_process in np.arange(no_roots):
...
#		b_dot[sel_ind]=b[sel_ind]*(alpha[sel_ind]*(b[sel_ind])-beta[sel_ind]*(b[sel_ind]**2)+gamma[sel_ind]*b[1-sel_ind]+epsilon[sel_ind]-delta[sel_ind]*(b[sel_ind]+b[1-sel_ind]))+(np.random.random()*2-1)*0.1
		...
		#b_dot[sel_ind]=b[sel_ind]*(5-b[sel_ind])*(alpha[sel_ind]*b[sel_ind]+beta[sel_ind]*b[1-sel_ind]+delta[sel_ind])+(np.random.random()*2-1)*0.2
			...
			b_dot_tmp=b_dot_tmp*(b[sel_ind]-alpha[sel_ind, sel_ind_process])
		
		for sel_other_ind in np.arange(no_eqs):
		
			#b_dot_tmp=b_dot_tmp+alpha[sel_ind, sel_other_ind]*b[sel_other_ind]+beta[sel_ind, sel_other_ind]*b[sel_ind]*b[sel_other_ind]
			
			b_dot_tmp=b_dot_tmp+beta[sel_ind, sel_other_ind]*b[sel_other_ind]
			
#		if sel_ind<2:
		
		b_dot[sel_ind]=b_dot_tmp#*(b[sel_ind]-0)*(10-b[sel_ind])#*(b_dot_tmp)#(b[sel_ind]-epsilon[sel_ind])*(delta[sel_ind]-b[sel_ind])*b_dot_tmp#+(np.random.random()*2-1)*0.2
...
#alpha=np.zeros([no_eqs, no_eqs])

#alpha[np.arange(no_eqs), np.random.permutation(no_eqs)]=np.random.random(no_eqs)*2-1
#alpha[np.arange(no_eqs), np.random.permutation(no_eqs)]=np.random.random(no_eqs)*2-1

#beta=np.zeros([no_eqs, no_eqs])

#beta[np.arange(no_eqs), np.random.permutation(no_eqs)]=np.random.random(no_eqs)*2-1
#beta[np.arange(no_eqs), np.random.permutation(no_eqs)]=np.random.random(no_eqs)*2-1

alpha=(np.random.random([no_eqs, no_roots])*2-1)*5

beta=(np.random.random([no_eqs, no_eqs])*2-1)#/no_eqs#np.zeros([no_eqs, no_eqs])#np.random.random([no_eqs, no_eqs])*2-1
...
for b1 in np.arange(-5.02,5.04,0.25):

	for b2 in np.arange(-5.02,5.04,0.25):
