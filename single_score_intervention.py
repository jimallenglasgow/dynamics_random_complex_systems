...
##to run: python3 single_score_intervention.py
...
full_interactions_long=np.reshape(interactions, (1,-1))

interactions_long=full_interactions_long[0, :]
...
full_inputs=np.append(growth_rate, growth_to_max_rate)

full_inputs=np.append(full_inputs, max_resources)

full_inputs=np.append(full_inputs, interactions_long)
...
##now increase the maximum for the "good" factor

best_node=factor_order[0]

print("Good factor = ", best_node)

##update the maximum

full_inputs[2*no_factors+best_node]=10
