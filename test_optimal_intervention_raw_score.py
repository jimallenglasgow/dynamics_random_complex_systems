...
possible_intervention_points=np.where(full_inputs!=0)[0]

print("Poss intervention points")

print(possible_intervention_points)

initial_full_inputs=full_inputs.copy()
...
for sel_intervention in possible_intervention_points:
	...
	min_intervention_value=0
	
	if sel_intervention>(3*no_factors):
	
		min_intervention_value=-10
		
	for intervention_value in np.arange(min_intervention_value, max_intervention_value, (max_intervention_value-min_intervention_value)/21):
		...
		intervention_info=[sel_intervention, intervention_value, total_score]
		
		all_intervention_values=np.append(all_intervention_values, intervention_info)

		print("Intervention point = ",sel_intervention,", value = ",np.round(intervention_value,2),", Total score = ",np.round(total_score,2))
		
all_intervention_values=np.reshape(all_intervention_values, (-1, 3))

print("Intervention info")

print(all_intervention_values)

best_intervention=np.argmin(all_intervention_values[:, 2])

print("best intervention = ",best_intervention)

best_values=all_intervention_values[best_intervention, :]
...
kick_type=4 ##1=value, 2=max, 3=interaction, 4=best found intervention, -1=no kick

best_intervention_point=best_values[0]
best_intervention_value=best_values[1]
...
if best_intervention_point<no_factors:
        ...
        print("Intervention type = interaction on factor ", np.mod(best_intervention_point, no_factors), "from factor ", np.floor((best_intervention_point-3*no_factors)/no_factors-1))
