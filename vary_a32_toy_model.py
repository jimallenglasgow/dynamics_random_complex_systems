...
##to run: python3 vary_a32_toy_model.py
...
#		ax[0].plot(full_t, full_z.T[:,[0,1]])
	...
	full_dynamics=np.vstack([full_t, full_z])
...
for a32 in np.arange(-8,2,0.1):
  ...
  #  print("closest_time = ",closest_time)
    ...
    intervention_node_value_before_intervention=final_values[sel_intervention_node+1, closest_time_id-1]
...
#    print("value_after_intervention = ", value_after_intervention)
    ...
    print("a32 = ", np.round(a32,2), ", target_node = ",np.round(value_before_intervention,2), ", intervention_node_value = ", np.round(intervention_node_value_before_intervention,2), ", intervention_effect = ", np.round(intervention_effect,2))
